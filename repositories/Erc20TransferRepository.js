 ...
 * @private
 */
Erc20TransferRepository.prototype._getAccountTransfersConditions = function (addr, options) {

    var where = { };

    where.$or = [{from : {$in: addr}}, {to : {$in: addr}}];
 ...
 * @return {*}
 */
Erc20TransferRepository.prototype.getCountAccountTransfers = function (addr, options, next) {
    return Erc20Transfer.count(this._getAccountTransfersConditions(addr, options), function(err, count) {
        return next(err, count);
    });
 ...
 * @param {Function} next
 */
Erc20TransferRepository.prototype.fetchAccountTransfers = function (addr, options, next) {

    return Erc20Transfer.find(this._getAccountTransfersConditions(addr, options), {}, {sort: {block_height: -1}, limit: options.limit, skip: options.offset}, function(err, transfers) {
        return next(err, transfers);
    });
