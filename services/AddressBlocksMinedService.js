...
AddressBlocksMinedService.prototype.processBlock = function (blockHeight, next) {
    ...
    return async.waterfall([function (callback) {
        return self.node.services.beerchaind.getJsonBlock(blockHeight, function (err, response) {
        ...
        switch (block.flags) {
            case beerchaincore.Block.PROOF_OF_STAKE:
                txHash = block.tx[1];
                break;

            case beerchaincore.Block.PROOF_OF_WORK:
                txHash = block.tx[0];
                break;
        }
