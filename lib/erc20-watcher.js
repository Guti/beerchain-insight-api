...
Erc20Watcher.prototype.start = function () {
    ...
    return async.waterfall([function (callback) {
        ...
        self.node.services.beerchaind.on('tip', self._rapidProtectedUpdateTip.bind(self));
        ...
        return async.eachSeries(block.txids, function (txHash, callback) {
            ...
            return self.node.getJsonRawTransaction(txHash, function (err, transaction) {
                ...
                return async.waterfall([function (callback) {
                    ...
                    return self.processCreate(blockHeight, block, txHash, txTime, receipt, createVouts, function (err) {
                        return callback(err);
                    });
 ...
 * @return {*}
 */
Erc20Watcher.prototype.processCreate = function (blockHeight, block, txHash, txTime, receipt, createVouts, callback) {
            ...
            if (vout.scriptPubKey && vout.scriptPubKey.type === 'create' && ContractsHelper.isErc20Contract(vout.scriptPubKey.hex)) {
                ...
                return async.waterfall([function (callback) {
                    ...
                    return self.erc20ContractsRepository.createOrUpdateTx(erc20Data, function (err, row) {
                        ...
                        self.common.log.info('[ERC20WATCHER] receipt', receipt);
                        ...
                        return self.processReceipt(receipt, txHash, txTime, block, function (err) {
                          self.common.log.info('[ERC20WATCHER] processReceipt', err);
                          return callback();
                        });
                        //return callback();
                    });
