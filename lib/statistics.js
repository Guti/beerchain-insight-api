...
StatisticsController.prototype.blockCount = function(req, res) {

    if (req.query.format === 'object') {
        return res.jsonp({
            blockcount: this.node.services.beerchaind.height
        });
    }

    return res.jsonp(this.node.services.beerchaind.height);
...
StatisticsController.prototype.totalSupply = function(req, res) {
    ...
    var supply = (new BigNumber(0)).plus(this.node.services.beerchaind.height * 25).plus(2 * premine).toString(10);
