...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.beerchaind.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
    return this.node.services.beerchaind.getMiningInfo(function(err, result) {
...
StatusController.prototype.getStakingInfo = function(callback) {
    return this.node.services.beerchaind.getStakingInfo(function(err, result) {
...
StatusController.prototype.getDgpInfo = function(req, res) {
  ...
  return this.node.services.beerchaind.getDgpInfo(function(err, result) {
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.beerchaind.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.beerchaind.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.beerchaind.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.beerchaind.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.beerchaind.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.beerchaind.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.beerchaind.height,
        error: null,
        type: 'beerchaincore node'
      };
